...
export async function createSequences(
  ...
  const validatedModels = imageModelsInput.map((m) => safeTextToImageModel(m));
  ...
  if (!primaryImageModel) {
    throw new Error('Expected resolveImageModels to return at least one model');
  }
  ...
  if (!primaryVideoModel) {
    throw new Error('Expected resolveVideoModels to return at least one model');
  }
  ...
  if (!primaryAudioModel) {
    throw new Error('Expected resolveAudioModels to return at least one model');
  }
